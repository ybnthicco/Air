...
try {
    ...
    $result2 = $dbh->query(
        "SELECT id FROM `movie` WHERE `actor` LIKE '%$actor%' ORDER BY `id` DESC"
    );
...
$result = $dbh->query(
    "SELECT id,title,cover,quality,rating FROM `movie` WHERE `actor` LIKE '%$actor%' ORDER BY `movie`.`id` DESC LIMIT $offset, $rowsperpage"
);
...
if ($currentpage > 1) {
    ...
    echo " <a class='page-numbers' href='$siteurl/actor/$actor/page/1/'><div class='pg'><<</div></a> ";
    $prevpage = $currentpage - 1;
    echo " <a class='page-numbers' href='$siteurl/actor/$actor/page/$prevpage/'><div class='pg'><</div></a> ";
} else {
    echo "<div class=\"pagination\">";
}
for ($x = $currentpage - $range; $x < $currentpage + $range + 1; $x++) {
    if ($x > 0 && $x <= $totalpages) {
        if ($x == $currentpage) {
            ...
            echo " <a class='page-numbers' href='$siteurl/actor/$actor/page/$x/'><div class='pg'>$x</div></a> ";
        }
    }
}
if ($currentpage != $totalpages) {
    $nextpage = $currentpage + 1;
    echo " <a class='page-numbers' href='$siteurl/actor/$actor/page/$nextpage/'><div class='pg'>></div></a> ";
    echo " <a class='page-numbers' href='$siteurl/actor/$actor/page/$totalpages/'><div class='pg'>>></div></a> ";
    echo "</div>";
} else {
    echo "</div>";
}
